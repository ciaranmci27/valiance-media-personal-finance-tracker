...
## All activity by default (2026-09-13, plan: glowing-wibbling-blum)

Unreviewed balanced drafts count in every owner-facing number (Wave parity). Month close ready gate, tax, year-end package and history parity stay reviewed-only. Exports capture what is on screen.

- [x] Step 1 SQL: migration `_accounting_all_activity.sql` (workspace(from,to,mode) with DROP of the 2-arg overload + grants; close_checklist balances in working mode; report guard in both modes + unbalanced_drafts counts <2 lines; report_command without the working gate) mirrored into schema.sql; test:accounting:schema
- [x] Step 2 DEFAULT_BOOK_MODE in reports.ts (zod default too); uncategorizedCents in account-balances.ts
- [x] Step 3 server: read.ts workspace + account-ledger pass mode; workspace.ts + route.ts pass the constant; local-test-client allowed params; export route comment
- [x] Step 4 client defaults: preload builders, reports initialFilter, accounts ledger query; verify-accounting-preload expectations
- [x] Step 5 Reports: Reviewed only checkbox replaces Book mode select; banner + footer + detail + document + model copy; shell openReviewInbox shared with Reports; books package caption
- [x] Step 6 Overview needs-review amount + Books: captions; bank panel captions; transactions sr-only copy
- [x] Step 7 tests: reports suite (workspace modes, working balance sheet, working capture + Scope), close suite (working balances vs observed), read-contracts pin
- [x] Verify: schema (16 comparisons), reports (97 + 39), close (25 + 15), preload, read-contracts (89), lint clean, build:accounting ok, test:accounting:all 50/52 (the two pre-existing DB-shape suites), demo browser check of Reports + Overview
- [ ] Follow-ups (not in this change): auto-post for rule-categorized drafts (SQL auto_post exists, no UI); duplicate-draft signal for out-of-window bank observations; admin-wide data export stays reviewed-only
- [x] Row clicks follow one rule (transactionRowAction in lib/accounting/transactions.ts): a draft opens its editor, anything else opens the detail view, from the description, category chip, mobile card, row background and Overview alike; the "..." menu keeps explicit Edit / View details. Entry link removed from the detail view footer. (2026-09-13)
- [x] Edit / Delete vocabulary (2026-09-13): Correct, correction, Reverse, Discard become Edit and Delete across the detail view, row menu, simple editor, journal editor, approval and review dialogs, transfer/register/payroll dialogs, the Deleted tab and owner-facing error messages. Notes replace required reasons; the client fills "Edited in Transactions" / "Deleted in Transactions" / "Restored in Transactions" when blank. History and audit labels keep reversal/correction wording.
- [x] Word-only edits stay in place (2026-09-13): the simple editor shows the payee picker for reviewed transactions and sends entry.context (memo, kind, payee) when amount, date, account, category and split memos are unchanged; money changes still save a new version via entry.correct. entry.context accepts memo (workflows.ts; SQL already did).
- [x] Deleted tab shows only deleted transactions (2026-09-13): earlier versions of edited transactions (rows with a replacement_entry_id) are excluded from the tab and labelled "Earlier version" in badges, subtitles, the entry picker and the detail view, which links to the current version.
- [x] Bulk edit (2026-09-13): any live row is selectable (drafts and reviewed; deleted rows and earlier versions are not). "Edit selected" opens one dialog with Edit: Category | Payee and a Change to picker. Payee saves in place on every target via entry.context (stored kind sent back unchanged; retired invoice receipts skipped). Category saves in place on drafts and as a new version on reviewed rows (entry.correct, note "Bulk edit in Transactions"); splits and transfers are skipped with a note. Review selected only takes drafts. Commands run in order; on a failure the first N targets are treated as landed so a retry does not resend them.
- [x] Edits keep bank evidence (2026-09-13): entry.correct no longer refuses matched entries. When the replacement keeps every matched line (same account, same amount) and the same date, the bank matches move to the new version; a changed bank side raises ACCT_BANK_SOURCE_CHANGED. Same in-flight migration (ledger_command appended); lifecycle suite covers carry-over and both refusals.
- [x] Bulk edit lines + review switch (2026-09-13): one line per field (Category, Payee), "Add another edit", per-line remove; "Mark selected transactions as reviewed" is a Toggle in the dialog and the Review selected toolbar button is gone. Saves run in phases: payees in place, then categories (drafts in one save, reviewed rows as new versions using the versions phase one returned), then entry.bulkpost in chunks of 50 for drafts that have a category. executeMany now returns per-command results. Account is deliberately not a bulk field: bank-fed accounts are the bank's fact; manual entries use the single editor.
- [x] Search inputs clear button (2026-09-13): every accounting search TextInput is `clearable` (X button, Escape clears): transactions, accounts, bank match, entry picker, manual registers, payroll runs, reconciliation x2, transfer dialogs.
- [x] Review badge follows the search and filters (2026-09-13): with any search or filter active the tab count is the number of matching unreviewed rows (the page total on the Review tab, a one-row needs_review read elsewhere); with none active it stays the whole inbox count.
- [x] History names the change (2026-09-13): an entry.context row in Receipts and history reads "Description changed", "Payee changed", "Type changed" (or a combination) from its before/after values instead of a fixed "Payee changed".
- [x] Bulk description (2026-09-13): Description is a third bulk field with a text "Change to"; drafts taking a category get it in the same save, everything else through entry.context in place. Up to three lines per bulk edit.
- [x] Inline rename in the list (2026-09-13): a pencil next to the description (visible on row hover or focus) swaps it for a small TextInput; Enter or blur saves through entry.context in place, Escape cancels; hidden in demo and on deleted rows.
- [x] Row menu in plain words (2026-09-13): Edit, Copy, Delete (Delete in Payroll for payroll entries; Restore on deleted rows). Details & receipts, View journal, Link as transfer, Record as transfer and Remember payee are gone from the menu; the detail view opens from the row, transfers come from the category picker, payee memory lives in the editor's checkbox. The unreachable link-as-transfer dialog and alias helper were removed.
- [x] One review checkmark (2026-09-13): accounting-review-check.tsx holds the round check button; the list rows and the editor footer both use it (the editor's Toggle is gone, with "Mark as reviewed" / "Reviewed on save" text beside the check). Pencil now sits right after the description text.
- [x] History shows old and new (2026-09-13): a context save lists each changed field as "old -> new" under its history line (description text, payee names, type).
- [x] Editor layout (2026-09-13): "Split" and "Journal entry" sit in the Payee row's second column, aligned with the field; labels shortened from "Split across categories" and "Edit as journal entry".
- [x] One payee form (2026-09-13): accounting-party-form.tsx holds PartyForm, PartyDialog and newParty(); the Payees page and the New payee option inside the transaction, journal and bulk editors all open the same "Add payee" dialog (name, relationship, default category, Advanced: contractor status, W-9, notes, archive). The editors' two-field quick dialog is gone; the context editor takes an accounts prop for the default-category picker.
- [x] History labels an unreview as "Marked unreviewed" (2026-09-13).
- [x] Manage without a Books tab (2026-09-13): the Books section, its rail group and BookSettings are gone; the transfer matching window lives under Advanced on Bank connections (saves preferences.save with the stored system-of-record values); the system-of-record control and its setup-guide step are removed (the database keeps primary_system as is); old manage/settings links land on Bank connections; rail order is Money in and out, Registers, Reference, Year end.
- [x] Bulk edit dialog polish (2026-09-13): rows share one 2:3 column grid so fields align across lines; an unset line shows a disabled "Change to" field ("Choose a field first") instead of loose text; "Choose a field" placeholder; remove buttons aligned to the inputs; dialog widened to sm:max-w-xl. Manage subtitle no longer mentions the Books tab.
- [x] Tax workpapers treatment labels (2026-09-13): treatmentLabels/treatmentLabel in lib/accounting/tax-workpapers.ts turn the stored tokens (other_deduction, gross_receipts, meals_50, officer_compensation...) into plain words on the support report screen and in the retained CSV/PDF. Unknown tokens lose their underscores.
- [x] Contacts (2026-09-13): "Payee" is "Contact" in every owner-facing string (Manage rail and page, transaction and journal editors, bulk edit, reports filter, rules and aliases, history labels, report descriptions, the retained document's filter row); internal names (payee_id, the payee filter value) are unchanged. The contact form adapts: Relationship is a Vendor / Customer / Both segmented control; vendors and Both get an "Independent contractor (1099)" switch that reveals contractor type and W-9; customers never see 1099 fields; Notes and Archive stay under Advanced.
- [x] Contact relationship (2026-09-13): Vendor / Payee, Customer, Contractor. Contractor = vendor kind plus a contractor status (individual by default) and reveals contractor type and W-9; the 1099 switch is gone. "Both" is no longer offered; a contact already saved as both keeps that choice. The contact picker groups contractors separately.
- [x] Transactions list no longer prints the contact name under the description (desktop subtitle and mobile card); the Deleted / Earlier version / Deletion markers stay.
- [x] Filters as a floating panel (2026-09-13): the Filters button opens a top-layer panel anchored under it (the inputs' popupChrome, popover="manual", via useInputPopup + createPortal) with the same fields, Reset filters and Done; clicks inside any open popup stay, clicks elsewhere or Escape close it. The inline accordion section is gone.
- [x] Sort is a menu button (2026-09-13): the toolbar Sort control is a ghost icon button opening the shared RowActionsMenu with the five orders and a check on the current one. RowActionsMenu accepts a custom trigger and a checked item.
- [x] Toolbar order is Sort, Filters, Search (2026-09-13): the three controls sit in that order on the transactions toolbar; the Filters panel and its portal moved with the button.
- [x] Header button reads New with a plus (2026-09-13): the accounting header menu trigger shows a plus icon and the word New, no chevron, with the accessible name New transaction; the overview empty state keeps Add transaction next to Connect a bank.
- [x] Detail view shows the contact (2026-09-13): the transaction detail dialog has a row under the amount labelled with the relationship (Vendor, Customer, Contractor, Vendor and customer) and the contact name, or Contact with No contact; transfers, payroll and opening balances show no row. partyRelationship and partyRelationshipLabel live in accounting-party-form.tsx and the form uses them too.
- [x] Affiliation tag in the list (2026-09-13): rows with a contact show a small default Badge after the description (Customer, Vendor, Contractor), no divider, adding the contact name only when the description does not already contain it (case-insensitive). The mobile card puts the badge beside the date. contactAffiliation lives in accounting-party-form.tsx. Demo books have no contacts, so the tag was verified by type check only.
- [x] Editing a saved contact answered 400 (2026-09-13): the manage read returns to_jsonb(parties) with contractor_classification, documentation_status, created_at and updated_at, and the form spread the whole contact into party.save, whose schema is strict. The form now sends the command field by field. Reproduced with a zod parse of a read-back contact (rejected) versus the explicit list (accepted).
- [x] Chart of accounts matches the Transactions list (2026-09-13): one toolbar row (type tabs left; Filters, Search accounts, Add account right), the title band is gone, Balances as of and Include archived live in the Filters panel, bank and card rows carry the institution mark, the code follows the name only when present, rows open the account activity on click, and a row menu (View activity, Edit) replaces the pencil. Balances on a past date count as a filter and retitle the balance column. The Filters button and panel are the shared FilterPopover (accounting-filter-popover.tsx), which Transactions now uses too.
- [x] No Journal label under amounts (2026-09-13): the transactions amount cell no longer prints Journal under entries without a single bank movement; the Category column already says Journal entry. The Transfer subline stays.
