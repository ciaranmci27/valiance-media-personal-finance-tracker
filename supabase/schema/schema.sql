...
-- View to get income entries with all amounts pivoted
CREATE OR REPLACE VIEW income_entries_with_totals AS
SELECT
  e.id,
  e.month,
  e.notes,
...
LEFT JOIN income_amounts a ON a.entry_id = e.id
WHERE e.deleted_at IS NULL
GROUP BY e.id, e.month, e.notes, e.deleted_at, e.created_at, e.updated_at
ORDER BY e.month DESC;

-- View for active expenses with monthly equivalent calculated
CREATE OR REPLACE VIEW expenses_with_monthly AS
SELECT
  *,
  CASE frequency
...
FROM expenses
WHERE deleted_at IS NULL AND is_active = true
ORDER BY expense_type, amount DESC;

-- ============================================================================
-- SCHEMA COMPLETE
-- ============================================================================
-- Next step: Run seed.sql to populate initial data
-- ============================================================================
