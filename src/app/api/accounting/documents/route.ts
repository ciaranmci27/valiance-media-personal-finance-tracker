...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await client.rpc("acct_documents_read", {
      p_id: id.data,
      p_offset: 0,
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const name = file.name.replace(/[\x00-\x1f\\/]/g, "_").slice(0, 250);
    const bytes = new Uint8Array(await file.arrayBuffer()),
      mime = documentMime(bytes, name),
      hash = createHash("sha256").update(bytes).digest("hex");
    const existing = await client.rpc("acct_documents_read", {
      p_id: id,
      p_offset: 0,
    });
    if (existing.error)
      throw new Error(accountingError(existing.error.message));
    let doc = (existing.data as DocumentList).documents[0];
    if (doc) {
      ...
      const prepared = await client.rpc("acct_execute", {
        p_key: id,
        p_command: {
    ...
    if (doc.state === "uploading") {
      ...
      const completed = await client.rpc("acct_execute", {
        p_key: randomUUID(),
        p_command: {
      ...
      if (completed.error) {
        const refreshed = await client.rpc("acct_documents_read", {
          p_id: id,
          p_offset: 0,
        });
        if (
