...
export function ExpenseDetailContent({
  ...
  const [category, setCategory] = React.useState<ExpenseCategory | null>(
    expense.category,
  );
  ...
  const handleDelete = async () => {
    const confirmed = await confirm({
      ...
      description:
        "This expense will be moved to trash. You can restore it later.",
      confirmLabel: "Delete",
      variant: "danger",
    });
    if (!confirmed) return;
  ...
  const displayAnnual = getMaskedValue(
    formatClean(annual),
    isHidden,
    isRevealed,
  );
        ...
        {isEditing ? (
          <>
            ...
            <Select
              ...
              value={category || ""}
              onChange={(value) =>
                setCategory((value as ExpenseCategory) || null)
              }
              placeholder="Select a category..."
              options={Object.entries(EXPENSE_CATEGORIES).map(
                ([value, label]) => ({
                  value,
                  label,
                }),
          ...
          <>
            ...
            <TextInput
              label="Category"
              value={category ? EXPENSE_CATEGORIES[category] : "None"}
              readOnly
            />
          </>
        )}
...
function HistoryRow({
  ...
  const showAmount =
    entry.event_type !== "paused" && entry.event_type !== "deleted";
