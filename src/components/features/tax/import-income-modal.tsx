...
export function ImportIncomeModal({
      ...
      try {
        if (isDemoMode()) {
          ...
          const sourceMap = new Map<
            string,
            { sourceId: string; name: string; total: number }
          >();
          for (const amount of demoIncomeAmountsPlain) {
            if (!entryIds.has(amount.entry_id)) continue;
            const source = demoIncomeSources.find(
              (s) => s.id === amount.source_id,
            );
            if (!source) continue;
          ...
          for (const a of amounts) {
            const name =
              (a.income_sources as unknown as { name: string })?.name ||
              "Unknown";
            const existing = sourceMap.get(a.source_id) || {
  ...
  const updateIncomeType = (index: number, newType: IncomeType) => {
    setSources((prev) =>
      prev.map((s, i) =>
        i === index
          ? { ...s, incomeType: newType, subjectToSe: newType === "1099" }
          : s,
      ),
    );
  };
        ...
        <div ref={contentRef} className="relative">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              These totals may reflect top, middle, or bottom-line revenue and
              may differ from your actual tax figures.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-2 max-h-80 overflow-y-auto">
            ...
            {!loading && sources.length > 0 && (
              <p className="text-xs text-muted-foreground px-1 pb-1">
                Select sources to pre-fill your tax income. You can adjust
                amounts after importing to reflect actual tax figures.
              </p>
            )}
