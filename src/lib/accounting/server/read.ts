...
export async function readAccounting(
  ...
  switch (view) {
    case "operate":
      return client.rpc("operate", {
        command: { key: p.key, command: p.command },
      });
    case "workspace": {
      ...
      if (!result.error && p.entry_id) {
        ...
        result.data = {
          ...(result.data as AccountingWorkspace),
          entries: entry.data ? [entry.data] : [],
          entry_count: entry.data ? 1 : 0,
        };
      }
      return result as {
    ...
    case "books-package-history":
      return client.rpc("books_package", {
        params: {
          ...p,
          view: view === "books-package-history" ? "history" : "preview",
        },
      });
    case "snapshot":
      return client.rpc("snapshot_read", { id: p.id });
    case "register":
      return client.rpc("transactions", {
        filter: p.filter,
        page: { offset: filter.offset ?? 0, limit: filter.limit ?? 50 },
      });
    case "evidence": {
    ...
    case "tax-source":
      return client.rpc("tax_source", { year: p.year, cutoff: p.through });
    case "setup-status":
      return client.rpc("setup_status", { year: p.year });
    case "contractors": {
      const result = await client.rpc("contractor_report", {
        year: filter.year ?? p.year,
        cutoff: filter.through ?? null,
      });
      if (result.error || !result.data) return result;
      const data = result.data as ContractorView,
        query = String(filter.query ?? "").toLocaleLowerCase("en-US"),
        offset = Number(filter.offset ?? 0);
      const rows = data.rows.filter(
        (row) =>
          (!filter.party || row.id === filter.party) &&
    ...
    case "payroll":
      return client.rpc("payroll", { view: p.filter });
    case "payroll-year":
      return client.rpc("payroll", {
        view: { year: p.year, through: p.through },
      });
    case "payroll-detail":
      return client.rpc("payroll", { view: { ...p, view: "detail" } });
    case "registers":
    case "register-detail":
    case "register-preview":
      return client.rpc("registers", {
        view: {
          ...p,
          view:
    ...
    case "support-report":
      return client.rpc("support_report", { params: p.filter });
    case "report":
    ...
    case "close":
      return client.rpc("close_checklist", { month: p.month });
    case "reconciliation":
    case "period-impact":
    case "transfers":
    case "cash-review":
      return client.rpc("context", { view, params: p });
    default:
      return { data: null, error: { message: "ACCT_INVALID_VIEW" } };
  }
}
