...
function isZero(cents: string | undefined | null): boolean {
  return (
    !cents || !/^-?(0|[1-9][0-9]*)$/.test(cents) || BigInt(cents) === BigInt(0)
  );
}
    ...
    if (!coverage) {
      ...
      if (payroll.employees.length > 0) {
        for (const employee of sortEmployees(payroll.employees)) {
          figures.push(
            ...employeeFigures(employee, payroll.through, null, "runs"),
          );
        }
      ...
      for (const employee of sortEmployees(coverage.employees)) {
        figures.push(
          ...employeeFigures(employee, through, documentId, "register"),
        );
      }
    }
    if (payroll.drafts > 0) {
      notes.push(
        `${plural(payroll.drafts, "draft payroll run")} not included yet`,
      );
    }
...
function employeeFigures(
  ...
  const where =
    basis === "register" ? "the verified register" : "posted payroll runs";
  ...
  const gross = has(employee.gross_cash_cents)
    ? employee.gross_cash_cents
    : null;
  const taxable = has(employee.federal_taxable_cents)
    ? employee.federal_taxable_cents
    : null;
  const wagesAmount = taxable ?? gross;
  const base = (value: string | null | undefined) =>
    has(value) ? value : wagesAmount;
  const ssBase = base(employee.social_security_wages_cents);
  const medicareBase = base(employee.medicare_wages_cents);
  const wagesOk =
    wagesAmount !== null && ssBase !== null && medicareBase !== null;
  out.push({
